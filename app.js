...
const K = {
    ...
    transferenciasUSD:CF_NS+'f_transferenciasUSD_v1',
...
//  se detecta IndexedDB vacía pero localStorage con datos viejos.
// ═══════════════════════════════════════════
const CF_DB_NAME = CF_NS + 'cf-data-db';
const K_ALL = [
    ...
    CF_NS+'f_presup_rubros_v1', CF_NS+'f_rubro_reporte4_v1', CF_NS+'f_rubro_reporte4_usd_v1',
    CF_NS+'f_presup_rubros_usd_v1', CF_NS+'f_rubros_usd_v1'
];
function cfDbOpen() {
...
function cfAplicarEstado(mapa) {
    ...
    if (get(CF_NS+'f_presup_rubros_v1')     !== undefined) listaPresupRubros    = get(CF_NS+'f_presup_rubros_v1');
    if (get(CF_NS+'f_rubro_reporte4_v1')    !== undefined) listaRubroReporte4  = get(CF_NS+'f_rubro_reporte4_v1');
    if (get(CF_NS+'f_rubro_reporte4_usd_v1')!== undefined) listaRubroReporte4USD = get(CF_NS+'f_rubro_reporte4_usd_v1');
    if (get(CF_NS+'f_presup_rubros_usd_v1') !== undefined) listaPresupRubrosUSD = get(CF_NS+'f_presup_rubros_usd_v1');
    if (get(CF_NS+'f_rubros_usd_v1')        !== undefined) listaRubrosUSD       = get(CF_NS+'f_rubros_usd_v1');
}
...
function guardar() {
    try {
        const entries = [
            ...
            [CF_NS+'f_presup_rubros_v1', listaPresupRubros], [CF_NS+'f_rubro_reporte4_v1', listaRubroReporte4],
            [CF_NS+'f_rubro_reporte4_usd_v1', listaRubroReporte4USD], [CF_NS+'f_presup_rubros_usd_v1', listaPresupRubrosUSD],
            [CF_NS+'f_rubros_usd_v1', listaRubrosUSD],
...
function cfActualizarVencimientosDesdeUltimoMes(){
    ...
    const aplicar = (listaActual, listaArchivada) => {
        listaActual.forEach(s=>{
            if(s.esCuota) return;
            if(s.pagado>0) return; // ya pagado este mes, no tocar
...
function renderPresupRubros() {
    ...
    listaRubros.forEach(r=>{
        ...
        html += '<label style="display:flex;align-items:center;gap:4px;font-size:10px;color:#94a3b8;cursor:pointer;margin-left:6px;">';
        html += '<input type="checkbox" '+(rubroEnReporte4(r)?'checked':'')+' data-rubro-r4="'+r.replace(/"/g,'&quot;')+'" onchange="toggleRubroReporte4(this)" style="margin:0;">';
        html += 'Reporte 4</label>';
        html += '</div></div>';
    });
    wrap.innerHTML = html;
}
...
let r4RubrosSelARS = null; // null = todavía no inicializado (se arma la primera vez que se abre el reporte)
...
// corsfix.com se sacó de la lista: para sitios que no son localhost exige whitelistear el
...
const APP_VERSION = 'v3.8.45-dev1';
...
const GTOKEN_SCOPE_KEY=CF_NS+'cf_gtoken_scope_v';
...
function driveEnsureFolder(token, cb) {
    ...
    .then(r=>r.json()).then(data=>{
        ...
        alert('No se encontró la carpeta "ControlFinanciero" en Drive todavía — subí un backup desde producción primero.');
    }).catch(()=>{});
}
