...
let listaRubroReporte4   = leer(CF_NS+'f_rubro_reporte4_v1')   || {}; // rubroNombre -> true/false. Ausente = incluido (default true).
...
function guardar() {
    try {
        ...
        localStorage.setItem(CF_NS+'f_rubro_reporte4_v1',    JSON.stringify(listaRubroReporte4));
        localStorage.setItem(CF_NS+'f_rubro_reporte4_usd_v1', JSON.stringify(listaRubroReporte4USD));
...
function nuevoMes(opts) {
    ...
    // Limpiar pesos
    listaServicios.forEach(s=>{ if(!s.esCuota){ s.fVto = s.fPago ? cfDiaAntes(s.fPago) : ''; } s.pagado=0; s.fPago=''; });
    ...
    // Limpiar USD
    listaServiciosUSD.forEach(s=>{ if(!s.esCuota){ s.fVto = s.fPago ? cfDiaAntes(s.fPago) : ''; } s.pagado=0; s.fPago=''; });
    listaCorrientesUSD=[];
...
let listaRubroReporte4USD = leer(CF_NS+'f_rubro_reporte4_usd_v1') || {}; // igual que listaRubroReporte4 pero para rubros USD
...
const APP_VERSION = 'v3.8.33-dev1';
